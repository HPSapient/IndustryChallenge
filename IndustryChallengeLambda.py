...
if os.path.exists("City_All_data.json"):
    with open("City_All_data.json") as f:
...
if __name__ == '__main__':
    ...
    for l in Locations:
        print(l)
        if l not in all_data.keys():
            businesses = []
            
            for i in range(10):        
                
                r =client.search_query(location = l, limit = 50, offset = i*50)    
                time.sleep(1)
                for b in r['businesses']:
                    temp_dict={}
                    if 'price' in b.keys():
                        temp_dict = {'name': b['name'],'price':len(b['price']),'rating':b['rating'],"categories":[x['alias'] for x in b['categories']]}
                    else:
                        temp_dict = {'name': b['name'],'price':0,'rating':b['rating'],"categories":[x['alias'] for x in b['categories']]}
                        
                    businesses.append(temp_dict)
            all_data[l] = businesses
            
import json
with open('City_All_data.json', 'w') as fp:
